...
class WPO_Menu_Cart_Pro_Data {
	...
	function __construct( $nav_menu_items, $menu_slug, $menu_args )	{
	...
	public function main_li_class() {
		...
		// Hide when empty
		if ( $this->item_data['cart_contents_count'] == 0 && ! isset( WPO_Menu_Cart_Pro()->main_settings['always_display'] ) && ! WPO_Menu_Cart_Pro()->main->is_block_editor() ) {
		...
		if ( isset( WPO_Menu_Cart_Pro()->main_settings['flyout_display'] ) ) {
			...
			// block submenu
			$classes .= ' has-child open-on-hover-click wp-block-navigation-submenu';
	...
	public function main_item_content() {
		...
		if ( !empty(WPO_Menu_Cart_Pro()->texts_links_settings['item_text'][$lang]['single']) && !empty(WPO_Menu_Cart_Pro()->texts_links_settings['item_text'][$lang]['plural']) ) {
			...
			/* translators: 1. item count, 2. custom singular name of the item, 3. custom plural name of the item (do not translate!) */
			$cart_contents = sprintf( _n( '%1$d %2$s', '%1$d %3$s', $item_count, 'wp-menu-cart-pro' ), $item_count, $item_text_setting['single'], $item_text_setting['plural'] );
			/* translators: 1. custom singular name of the item, 2. custom plural name of the item (do not translate!) */
			$item_text = sprintf( _n( '%1$s', '%2$s', $item_count, 'wp-menu-cart-pro' ), $item_text_setting['single'], $item_text_setting['plural'] );
		}
	...
	public function submenu_item_remove( $submenu_item_data ) {
		...
		$icon = apply_filters( 'wpmenucart_submenu_item_remove_icon', WPO_Menu_Cart_Pro()->plugin_url().'/assets/images/icon-remove.svg' );
		$alt  = __('Remove cart item', 'wp-menu-cart-pro');

		// The remove icon
		$submenu_item_remove = '<div class="wpmenucart-product-remove" data-key="'.$key.'"><img class="wpmenucart-product-remove-icon" src="'.$icon.'" alt="'.$alt.'"></div>';
		$submenu_item_remove = ( function_exists('WC') || function_exists('EDD') ) && isset(WPO_Menu_Cart_Pro()->main_settings['flyout_remove_items']) && WPO_Menu_Cart_Pro()->main_settings['flyout_remove_items'] != '' ? $submenu_item_remove : null;

		return apply_filters( 'wpmenucart_submenu_item_remove', $submenu_item_remove, $this->item_data, $submenu_item_data, WPO_Menu_Cart_Pro()->main_settings);
	}
	...
	public function floating_div_class() {
		$classes = 'wpmenucart-floating-cart';

		// display on small devices only
		if( isset( WPO_Menu_Cart_Pro()->main_settings['floating_cart'] ) && WPO_Menu_Cart_Pro()->main_settings['floating_cart'] == 'small-devices' ) {
			$classes .= ' small-devices-only';
		}

		// cart icon style class
		if ( isset( WPO_Menu_Cart_Pro()->main_settings['floating_cart_style'] ) ) {
			$style_class = WPO_Menu_Cart_Pro()->main_settings['floating_cart_style'];
			$classes    .= ' '.$style_class;

			// cart icon position class
			if ( isset(WPO_Menu_Cart_Pro()->main_settings['floating_cart_position']) ) {
				$position_class = WPO_Menu_Cart_Pro()->main_settings['floating_cart_position'];
		...
		return apply_filters( 'wpmenucart_floating_div_class', $classes, $this->item_data, WPO_Menu_Cart_Pro()->main_settings);
	}

	public function floating_div_style() {
		return apply_filters( 'wpmenucart_floating_div_style', '', $this->item_data, WPO_Menu_Cart_Pro()->main_settings);
	}

	public function floating_a_class() {
		if ($this->item_data['cart_contents_count'] == 0) {
			$classes = 'wpmenucart-floating-contents empty-wpmenucart-floating-visible';
		} else {
			$classes = 'wpmenucart-floating-contents';
		}

		return apply_filters( 'wpmenucart_floating_a_class', $classes, $this->item_data, WPO_Menu_Cart_Pro()->main_settings);
	}

	public function floating_item_content() {
		$item_count    = $this->item_data['cart_contents_count'];
		$cart_contents = strval( $item_count );
			
		if (isset(WPO_Menu_Cart_Pro()->main_settings['icon_display'])) {
			if (!empty(WPO_Menu_Cart_Pro()->main_settings['custom_icon'])) {
				$custom_image = wp_get_attachment_image_src(WPO_Menu_Cart_Pro()->main_settings['custom_icon']);
				$floating_item_icon = '<img src="'.$custom_image[0].'" class="wpmenucart-custom-icon">';
			} else {
				$icon = isset(WPO_Menu_Cart_Pro()->main_settings['cart_icon']) ? WPO_Menu_Cart_Pro()->main_settings['cart_icon'] : '0';
				$floating_item_icon = sprintf('<i class="wpmenucart-icon-shopping-cart-%s" role="img" aria-label="%s"></i>', $icon, __( 'Cart','woocommerce' ) );
			}
		} else {
			$floating_item_icon = '';
		}
		
		$floating_item_a_content = $floating_item_icon.'<span class="cartcontents">'.$cart_contents.'</span>';

		return apply_filters( 'wpmenucart_floating_item_content', $floating_item_a_content, $this->item_data, WPO_Menu_Cart_Pro()->main_settings);
	}
