...
class WPO_Menu_Cart_Pro_Settings_Callbacks extends WPO_Settings_Callbacks_2 {
	...
	/**
	 * Displays a color picker in settings field along
	 * with a checkbox to enable/disable this option
	 *
	 * @param array   $args settings field args
	 */
	public function optional_color_picker_element_callback( $args ) {
		extract( $this->normalize_settings_args( $args ) );
		
		$options = get_option( $option_name );
		$enabled_option_key = "{$id}_enabled";
		$enabled_option = isset( $options[$enabled_option_key] ) ? $options[$enabled_option_key] : '';

		$html = sprintf( 
			'<div class="optional_color_picker">
				<input type="checkbox" id="%1$s" name="%2$s[%1$s]" value="1" %3$s />
				<input type="color" id="%4$s" name="%5$s" value="%6$s" style="display:none;" />
			</div>', 
			$enabled_option_key, $option_name, checked( 1, $enabled_option, false ), $id, $setting_name, $current 
		);
